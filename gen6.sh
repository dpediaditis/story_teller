...
say "1. Seed (plain statements — psql cannot interpolate into a DO block)"
...
[ -n "$P" ] || die "auth user insert failed"
echo "parent    $P" | tee -a "$OUT"
...
[ -n "$D" ] || die "drawing insert failed"
echo "drawing   $D" | tee -a "$OUT"
...
[ -n "$CH" ] || die "character insert failed"
echo "character $CH" | tee -a "$OUT"
...
say "2. Claim quota + enqueue"
# set_config and the claim must share one session, so both go in one invocation.
PSQL <<EOSQL 2>&1 | tee -a "$OUT"
select set_config('request.jwt.claims','{"sub":"$P","role":"authenticated"}',false);
select public.claim_story_quota('$C'::uuid, array['$CH']::uuid[], 'space','funny','short','cutout_rerender','e2e','e2e-$(date +%s)');
EOSQL
echo "queued    $(PSQL -c 'select count(*) from pgmq.q_papercub_generation;')" | tee -a "$OUT"
[ "$(PSQL -c 'select count(*) from pgmq.q_papercub_generation;')" -ge 1 ] || die "nothing enqueued"
...
for i in $(seq 1 72); do
  ...
  echo "  t+$((i*5))s  ${ST:-(no job row)}" | tee -a "$OUT"
