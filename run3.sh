...
say "0. What is the timeout, actually?"
grep -n "DEFAULT_TIMEOUT_MS" services/worker/src/providers/gemini.ts | tee -a "$OUT"

say "1. Raise it, and report each attempt"
python3 - <<'PY' | tee -a "$OUT"
import re, pathlib
p = pathlib.Path("services/worker/src/providers/gemini.ts"); s = p.read_text()
m = re.search(r"const DEFAULT_TIMEOUT_MS\s*=\s*([0-9_]+)", s)
print(f"  was: {m.group(1) if m else 'NOT FOUND'}")
if m:
    # Image generation legitimately takes 10-30s; a cover plus six pages needs
    # generous headroom. 90s per request, with the retry budget on top.
    s = re.sub(r"(const DEFAULT_TIMEOUT_MS\s*=\s*)[0-9_]+", r"\g<1>90_000", s)
# Surface WHY a retry happened — "aborted" alone hid a 6s timeout for two runs.
old = "        const error = new GeminiError(res.status, text);"
new = ("        const error = new GeminiError(res.status, text);\n"
       "        console.error(`[gemini] ${model} attempt ${attempt} -> HTTP ${res.status}`);")
if old in s and "[gemini] ${model} attempt" not in s:
...
select public.claim_story_quota('$C'::uuid, array['$CH']::uuid[], 'space','funny','short','cutout_rerender','e2e','run3-$(date +%s)');
EOSQL
...
say "5. Attempt-level detail"
grep -a "\[gemini\]" /tmp/pc-worker.log 2>/dev/null | tail -12 | tee -a "$OUT"
grep -a '"level":"error"' /tmp/pc-worker.log 2>/dev/null | tail -3 | cut -c1-300 | tee -a "$OUT"
