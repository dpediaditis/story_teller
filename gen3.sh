...
say "1. Push the schema"
npx --yes supabase db push --db-url "$SUPABASE_DB_URL" 2>&1 | tail -20 | tee -a "$OUT"
N=$(PSQL -tA -c "select count(*) from information_schema.tables where table_schema='public';" | tr -d ' ')
echo "public tables now: $N" | tee -a "$OUT"
[ "${N:-0}" -ge 15 ] || die "schema did not apply (only $N tables)"
PSQL -tA -c "select 'extensions: '||string_agg(extname,', ') from pg_extension where extname in ('pgmq','pg_cron');" | tee -a "$OUT"

say "2. Seed a character with a real drawing"
...
head -c 200 /tmp/pc-up.json | tee -a "$OUT"; echo | tee -a "$OUT"

PSQL -tA -v parent="$PARENT" -v key="$KEY" <<'SQL' 2>&1 | tee -a "$OUT"
do $$
...
for i in $(seq 1 72); do
  ...
  ST=$(PSQL -tA -c "select status||' / '||coalesce(stage,'')||'  pages='||coalesce(pages_completed,0)||'/'||coalesce(pages_total,0)||'  cost='||coalesce(cost_cents,0)||'c' from public.generation_jobs order by created_at desc limit 1;" 2>/dev/null | tr -d '\r')
