-- Fixes a free-story bypass introduced in 20260826120000.
--
-- `select tier, topup_stories_remaining into v_tier, v_topup from subscriptions
-- where parent_id = ...` sets BOTH variables to NULL when no row exists — it
-- does not leave the declared defaults in place. A user with no subscriptions
-- row (i.e. every free user) therefore evaluated:
--
--     stories_used >= limit AND v_topup <= 0
--  => 1 >= 1            AND NULL <= 0
--  => true              AND NULL
--  => NULL              -- not true, so the guard never fired
--
-- The claim then took the "consumed a topup" branch and decremented a
-- subscriptions row that does not exist (a silent no-op), leaving stories_used
-- untouched. Every free user could mint unlimited free stories, bounded only by
-- the $3.85 measured ceiling.
--
-- Caught by an adversarial test, not by reading: the SQL looks correct.
...
begin
  ...
  -- THE FIX. No subscriptions row means NULL, not the declared default.
