-- The language a story is written and read aloud in, chosen per story.
--
-- It used to come from parent_accounts.locale, defaulted to 'en', and was never
-- offered to anyone. A language is NOT a premium feature
-- (packages/shared/src/languages.ts): a family that cannot read the free story
-- has not been given one. So there is no tier check for it — only a recorded
-- choice — and the reading-level gate is calibrated per language, because its
-- verdict is refundable and an English threshold applied to German compounds
-- would fail every story for a four-year-old.
--
-- claim_story_quota is redefined WHOLE, derived from the body in
-- 20260829152616 exactly as that file's header instructs. This file is now the
-- one place it is defined.

create type public.story_locale as enum (
  'en-GB', 'es-ES', 'de-DE', 'fr-FR', 'it-IT', 'el-GR', 'nl-NL'
);

alter table public.stories
  add column if not exists locale public.story_locale not null default 'en-GB';

comment on column public.stories.locale is
  'Language the story was WRITTEN in, not just narrated. Drives the writer '
  'prompt, the narration, and the reading-level thresholds (languages.ts).';
...
create or replace function public.claim_story_quota(
  ...
  p_locale public.story_locale default 'en-GB'
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  -- No locale check, deliberately. Every language is free (languages.ts): a
  -- family that cannot read the free story has not been given one.
  ...
  insert into public.stories (
    ...
    p_model_bundle_version, p_voice_id, p_locale
