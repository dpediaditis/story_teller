...
say "1. Pin models by measured health, not by version number"
# Measured 26 Aug: 3.7-flash 2/8, 3.1-flash-lite 10/10 (~1s), 3.5-flash 10/10 (~9s).
# Newest != most available. Gates and vision go to the fast model; story prose
# goes to the stronger one, per PLAN.html §8 — text is <2% of cost and prose
# quality is a real differentiator, so it is the one place worth paying for.
python3 - <<'PY' | tee -a "$OUT"
import re, pathlib
env = pathlib.Path(".env"); s = env.read_text()
want = {"GEMINI_TEXT_MODEL":"gemini-3.5-flash","GEMINI_VISION_MODEL":"gemini-3.1-flash-lite"}
for k,v in want.items():
    if re.search(rf"^{k}=.*$", s, re.M): s = re.sub(rf"^{k}=.*$", f"{k}={v}", s, flags=re.M)
    else: s += f"\n{k}={v}\n"
env.write_text(s)
cfg = pathlib.Path("services/worker/src/config.ts"); c = cfg.read_text()
for k,v in want.items():
    c = re.sub(rf"({k}:\s*z\.string\(\)\.default\(')[^']*(')", rf"\g<1>{v}\g<2>", c)
cfg.write_text(c)
print(f"  text={want['GEMINI_TEXT_MODEL']}  vision/gates={want['GEMINI_VISION_MODEL']}")
PY
set -a; . ./.env; set +a
...
select public.claim_story_quota('$C'::uuid, array['$CH']::uuid[], 'space','funny','short','cutout_rerender','e2e','run-$(date +%s)');
EOSQL
...
say "5. Log"
grep -a '"level":"error"\|"level":"warn"' /tmp/pc-worker.log 2>/dev/null | tail -6 | cut -c1-320 | tee -a "$OUT"
